define([ "intern!tdd", 
	...
	with(tdd){
		suite("Model.removeLeft", function(){
			...
			test("当没有任何内容时，removeLeft什么也不做", function(){
			...
			test("当有一个text文本时，removeLeft后也删除text节点", function(){
			...
			test("当text节点中有三个字符时，删除最后一个", function(){
			...
			test("当text节点中有三个字符时，删除中间一个", function(){
				model.setData({data:"你我他"});
				model.moveLeft();
			...
			test("当text节点中有三个字符时，删除最前一个", function(){
				model.setData({data:"你我他"});
				model.moveLeft();
				model.moveLeft();
			...
			test("当在一个text节点后，有一个存储着1的math节点，在math节点后执行removeLeft后，text节点获取焦点", function(){
			...
			test("当一个text文本后，有math时，在text文本后执行removeLeft后，math获取焦点", function(){
				model.setData({data:"和"});
				model.setData({data:"1"});
				model.moveLeft();
			...
			test("当有一个text节点，其后紧跟math节点，如果text节点中有两个字符，则text之后执行removeLeft后，就近text获取焦点。", function(){
			...
			test("当math节点后，有一个text节点，两个中都只有一个字符，删除text节点后，math节点的最后一个子节点获取焦点", function(){
				model.setData({data:"1"});
				model.setData({data:"a"});
			...
			test("当math节点中任何内容的token节点，只要其中没有内容，就删除节点，并让前一个节点获取焦点。", function(){
				model.setData({data:"1"});
				model.setData({data:"+"});
			...
			test("删除一个unicode字符，这里使用除号测试。因为现在只有操作符以unicode表示，所以一个操作符用一个mo封装，removeLeft遇到mo直接删除", function(){
				model.setData({data:"&#xD7;"});
  				t.is("&#xD7;",model.removeLeft());
  				t.is("/root/line[1]", model.getPath());
			...
			test("删除第一个空行时，保留该行对应的div；删除其余的空行时，则删除对应的div", function(){
			...
			// 根据当前插入点获取其余XXXXXX信息
			// 每种移动都有多种临界点，都需要测试
			test("在model中右移", function(){
				
			});
			
			test("在model中左移", function(){
				
			});
			
			test("在model中下移", function(){
