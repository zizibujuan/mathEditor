define([ "intern!tdd", 
	...
	//		2.节点是layout节点，删除该节点后，显示占位符
	with(tdd){
		suite("Model.removeLeft.frac 左删除分数中的内容或整个分数", function(){
			...
			test("光标在整个分数之后，左删除删除整个分数，math中只有一个分数节点", function(){
			...
			test("光标在整个分数之后，左删除删除整个分数，math中只有一个分数节点,并且分数节点被mstyle封装", function(){
				// mstyle是必须要考虑的
  				model.loadData("<root><line><math>" +
  						"<mstyle>" +
	  						"<mfrac>" +
			...
			test("光标在整个分数之后，左删除删除整个分数，分数前是一个token节点", function(){
				model.loadData("<root><line><math>" +
  						"<mn>1234</mn>" +
  						"<mfrac><mrow><mn>1</mn></mrow><mrow><mn>2</mn></mrow></mfrac>" +
  				"</math></line></root>");
			...
			test("光标在整个分数之后，左删除删除整个分数(分数外有mstyle节点)，分数前是一个token节点", function(){
				model.loadData("<root><line><math>" +
  						"<mn>1234</mn>" +
  						"<mstyle>" +
  						"<mfrac><mrow><mn>1</mn></mrow><mrow><mn>2</mn></mrow></mfrac>" +
			...
			test("光标在整个分数之后，左删除删除整个分数，分数前是一个layout节点", function(){
				model.loadData("<root><line><math>" +
  						"<mfrac><mrow><mn>1</mn></mrow><mrow><mn>2</mn></mrow></mfrac>" +
			...
			test("光标在整个分数之后，左删除删除整个分数，分数前是一个layout节点(两个layout节点都被mstyle封装)", function(){
				model.loadData("<root><line><math>" +
			...
			test("光标在整个分数之后，左删除删除整个分数，分数前没有节点，分数后有一个token节点", function(){
				model.loadData("<root><line><math>" +
  						"<mfrac><mrow><mn>1</mn></mrow><mrow><mn>2</mn></mrow></mfrac>" +
  						"<mn>1234</mn>" +
  				"</math></line></root>");
			...
			test("光标在整个分数之后，左删除删除整个分数（分数被mstyle封装），分数前没有节点，分数后有一个token节点", function(){
				model.loadData("<root><line><math>" +
  						"<mstyle><mfrac><mrow><mn>1</mn></mrow><mrow><mn>2</mn></mrow></mfrac></mstyle>" +
  						"<mn>1234</mn>" +
  				"</math></line></root>");
			...
			test("光标在整个分数之后，左删除删除整个分数，分数前没有节点，分数后有一个layout节点", function(){
				model.loadData("<root><line><math>" +
  						"<mfrac><mrow><mn>1</mn></mrow><mrow><mn>2</mn></mrow></mfrac>" +
  						"<mfrac><mrow><mn>3</mn></mrow><mrow><mn>4</mn></mrow></mfrac>" +
  				"</math></line></root>");
  				...
  				assert.equal("mfrac", focusNode.nodeName);
  				assert.equal(0, model.getOffset());
  				assert.equal("34", dripLang.getText(focusNode));// 确保是第二个分数
  				assert.equal(1, line.firstChild.childNodes.length)// 确保mfrac节点从math中删除
			});
			
			test("光标在整个分数之后，左删除删除整个分数，分数前没有节点，分数后有一个layout节点(两个layout节点都被mstyle封装)", function(){
				model.loadData("<root><line><math>" +
  						"<mstyle><mfrac><mrow><mn>1</mn></mrow><mrow><mn>2</mn></mrow></mfrac></mstyle>" +
  						"<mstyle><mfrac><mrow><mn>3</mn></mrow><mrow><mn>4</mn></mrow></mfrac></mstyle>" +
  				"</math></line></root>");
			...
			test("删除分母,光标在分母上，分母里没有内容，左删除删掉分数结构，留下分子中的内容，光标在后面，分子的最后一个节点是token节点", function(){
				model.loadData("<root><line><math>" +
			...
			test("删除分母,光标在分母上，分母里没有内容，左删除删掉分数结构，留下分子中的内容，光标在后面，分子的最后一个节点是token节点(分数被mstyle封装)", function(){
				model.loadData("<root><line><math>" +
			...
			test("删除分母,光标在分母上，分母里没有内容，左删除删掉分数结构，留下分子中的内容，光标在后面，分子的最后一个节点是layout节点", function(){
				model.loadData("<root><line><math>" +
			...
			test("删除分母,光标在分母上，分母里没有内容，左删除删掉分数结构，留下分子中的内容，光标在后面，分子的最后一个节点是layout节点(分数外用mstyle封装)", function(){
				model.loadData("<root><line><math>" +
  						"<mstyle>" +
	  						"<mfrac>" +
			...
			test("删除分母,光标在分母上，分母里没有内容，左删除删掉分数结构，留下分子中的内容，光标在后面，分子的最后一个节点是layout节点(分数外用mstyle封装,分子也用mstyle封装)", function(){
				model.loadData("<root><line><math>" +
  						"<mstyle>" +
	  						"<mfrac>" +
			...
			test("删除分子,光标在分子上，分子里没有任何内容，左删除删掉分子结构，留下分母中的内容，光标在前面，分母的第一个节点是token节点", function(){
				model.loadData("<root><line><math>" +
  						"<mfrac>" +
	  						"<mrow><mn class=\"drip_placeholder_box\">8</mn></mrow>" +
	  						"<mrow><mn>12</mn></mrow>" +
			...
			test("删除分子,光标在分子上，分子里没有任何内容，左删除删掉分子结构，留下分母中的内容，光标在前面，分母的第一个节点是layout节点", function(){
				model.loadData("<root><line><math>" +
  						"<mfrac>" +
	  						"<mrow><mn class=\"drip_placeholder_box\">8</mn></mrow>" +
	  						"<mrow><msup><mrow>1</mrow><mrow>2</mrow></msup></mrow>" +
